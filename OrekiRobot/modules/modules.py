...
from OrekiRobot import OREKI_MOD, tbot
    ...
    if "__handlers__" in dir(imported_module):
        ...
        for handler in handlers:
            if not isinstance(handler, tuple):
                ...
                OREKI_MOD.add_handler(handler_name, priority)
    else:
    ...
    if "__handlers__" in dir(imported_module):
        ...
        for handler in handlers:
            if isinstance(handler, bool):
                ...
                OREKI_MOD.remove_handler(handler)
            elif isinstance(handler[0], collections.Callable):
                callback, telethon_event = handler
                tbot.remove_event_handler(callback, telethon_event)
            else:
                handler_name, priority = handler
                OREKI_MOD.remove_handler(handler_name, priority)
    else:
...
def listmodules(update: Update, context: CallbackContext):
    ...
    for helpable_module in HELPABLE:
        ...
        file_name = file_info.__name__.rsplit("OrekiRobot.modules.", 1)[1]
...
OREKI_MOD.add_handler(LOAD_HANDLER)
OREKI_MOD.add_handler(UNLOAD_HANDLER)
OREKI_MOD.add_handler(LISTMODULES_HANDLER)
