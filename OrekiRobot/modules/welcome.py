...
def send(update, message, keyboard, backup_message):
    ...
    if cleanserv:
        try:
            OREKI_MOD.bot.delete_message(chat.id, update.message.message_id)
        except BadRequest:
            pass
        reply = False
    try:
    ...
    if should_goodbye:
        ...
        if left_mem:
            ...
            if left_mem.id == OWNER_ID:
                update.effective_message.reply_video(
                    OREKI_VID, caption="Oooopppss! My Master left..", reply_to_message_id=reply
                )
                return
...
WELC_HELP_TXT = (
    ...
    "buttons.\n"
    f"To create a button linking to your rules, use this: `[Rules](buttonurl://t.me/{OREKI_MOD.bot.username}?start=group_id)`. "
...
OREKI_MOD.add_handler(NEW_MEM_HANDLER)
OREKI_MOD.add_handler(LEFT_MEM_HANDLER)
OREKI_MOD.add_handler(WELC_PREF_HANDLER)
OREKI_MOD.add_handler(GOODBYE_PREF_HANDLER)
OREKI_MOD.add_handler(SET_WELCOME)
OREKI_MOD.add_handler(SET_GOODBYE)
OREKI_MOD.add_handler(RESET_WELCOME)
OREKI_MOD.add_handler(RESET_GOODBYE)
OREKI_MOD.add_handler(CLEAN_WELCOME)
OREKI_MOD.add_handler(WELCOME_HELP)
OREKI_MOD.add_handler(WELCOMEMUTE_HANDLER)
OREKI_MOD.add_handler(CLEAN_SERVICE_HANDLER)
OREKI_MOD.add_handler(BUTTON_VERIFY_HANDLER)
OREKI_MOD.add_handler(WELCOME_MUTE_HELP)
